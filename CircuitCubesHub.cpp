...
{
    _pRemoteCharacteristicWrite->writeValue("b");
    std::string value = _pRemoteCharacteristicNotify->readValue();
    log_i("read %s", value.c_str());

    // byte command[1] = {(byte)CircuitCubesCommandType::BATTERY};
    // WriteValue(command, 1);

    // NimBLEAttValue value = _pRemoteCharacteristicNotify->readValue();
    // log_w("battery level: %s", gexHexString2((byte*)value.data(), value.length()).c_str());
    ...
    bool isNotify)
{
    ...
    log_w("data: [%s]", gexHexString2(pData, length).c_str());
}
